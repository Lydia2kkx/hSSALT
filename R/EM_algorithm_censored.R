...
EM_algorithm_censored <- function(ind, data, d, N, parameter_starts, tol){
  #our input of initial guess can be a vector of three elements or a matrix (data frame) of three columns
  ...
  p_helper <- p1 #theta21 should be smaller than theta22 in the output
  if(rate1 <= rate2){
    theta21 <- 1/rate2
    theta22 <- 1/rate1
    p1 <- p2
