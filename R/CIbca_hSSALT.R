...
CIbca_hSSALT<- function(data, n, censoring, tau, r , monitoring, alpha, B, theta_21,
  ...
  p_bootstrap_ordered <- stl_sort(bootstrap_distri$pi1)
  ...
  z0p <- qnorm(sum(p_bootstrap_ordered < p1_em) / B)
  z021 <- qnorm(sum(theta21_bootstrap_ordered < theta21_em) / B)
  z022 <- qnorm(sum(theta22_bootstrap_ordered < theta22_em) / B)
  ...
  pij <- c(rep(p1_em, n1), max_loglik_re_df$pi1)
  pi <- mean(pij)
  ap <- sum((pi - pij)^3) / (6 * (sum((pi - pij)^2))^1.5)
  
  a21ij <- c(rep(theta21_em, n1), max_loglik_re_df$theta21)
  a21i <- mean(a21ij)
  a21 <- sum((a21i - a21ij)^3) / (6 * (sum((a21i - a21ij)^2))^1.5)
  
  a22ij <- c(rep(theta22_em, n1), max_loglik_re_df$theta22)
  a22i <- mean(a22ij)
  ...
  return(as.list(CI_theta_1 = c(theta1_low, theta1_up),
